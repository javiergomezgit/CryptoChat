...
final class UserDatabaseController {
    ...
    public func getAllMessagesForConversations(with chatID: String, encryptedText: Bool, completion: @escaping (Result<[Message], Error>) -> Void) {
        ...
        //sending non-encrypted for testing purposes, will send simple not encrypted messages
    ...
    public func sendChat(to chatID: String, usernameFriend: String, friendID: String, firstMessage: Message, friendPhotoURL: String, passwordEncrypt: String, isContact: Bool, completion: @escaping(Bool) -> Void) {
    ...
    public func isContactTemporolName(chatID: String, completion: @escaping (Bool) -> Void) {
        database.child("chats/\(chatID)/settings").observeSingleEvent(of: .value) { snapshot in
            ...
            let isContact = settings["is_friend"] as? Bool
            completion(isContact!)
        }
    }

    public func addNewContact(friendID: String, currentID: String, completion: @escaping (Bool) -> Void) {
        let isContactText = true
        database.child("users/\(currentID)/my_friends/\(friendID)/is_friend").setValue(isContactText) { error, ref in
            if let error = error {
