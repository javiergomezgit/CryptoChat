            ...
            do {
                try contactStore.enumerateContacts(with: request) { (contact, stop) in
                    ...
                    for phoneNumber in contact.phoneNumbers {
                        ...
                        let contactUser = ContactNumber(
                            firstName: contact.givenName,
                            lastName: contact.familyName,
                            profilePhoto: contact.imageData,
                            phoneNumber: phoneNumber,
                            labelNumber: localizedLabel
                        )
                ...
                let sortedContacts = contactsListNoSorted.sorted { $0.firstName < $1.firstName }

                // Switch back to the main thread to update UI
                DispatchQueue.main.async {
                    self.contactsList = sortedContacts
    ...
    @objc private func inviteFriendsSMS() {
        ...
        if (MFMessageComposeViewController.canSendText()) {
            ...
            self.present(controller, animated: true) {
                UserDatabaseController.shared.searchForIDsWithPhones(phoneNumbers: self.phonesAsUsers) { resultFriends in
            ...
            present(ShowAlert.alert(type:.simpleError, error: "There was an error, try again later"), animated: true)
        }
    }
