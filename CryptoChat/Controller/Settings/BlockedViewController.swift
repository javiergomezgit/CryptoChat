...
class BlockedViewController: UIViewController {
    ...
    private func getBlockedFriends() {
        ...
        UserDatabaseController.shared.getBlockedUsers(userID: currentUserID) { blockedFriends in
    ...
    @objc private func unlockedTapped(sender: UIButton) {
        ...
        let alert = UIAlertController(title: "Unblock", message: "Are you sure you want to unblock this user?", preferredStyle: .actionSheet)
        alert.addAction(UIAlertAction(title: "Unblock", style: .destructive, handler: { _ in
            UserDatabaseController.shared.unblockFriend(currentID: self.currentUserID, friendID: friendToUnlock.friendID) { success in
                if success {
                    self.getBlockedFriends()
                } else {
