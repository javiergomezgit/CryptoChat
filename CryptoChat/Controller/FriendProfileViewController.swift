...
//  CryptoChat
//
//  Created by Javier Gomez on 8/03/21.
//
...
class FriendProfileViewController: UIViewController {
    ...
    @IBOutlet weak var viewRequestFriend: UIView!
    ...
    public var friend = Friend(idFriend: "", blocked: false, chatID: "", friendsSince: "", isFriend: false, phoneNumberFriend: "", photoURLFriend: "", usernameFriend: "")
    ...
    override func viewDidLoad() {
        ...
        DatabaseMng.shared.getEncryptPass(chatID: friend.chatID) { password in
            if password != nil {
        ...
        DatabaseMng.shared.getPhoneWithID(userID: friend.idFriend) { phoneNumber in
        ...
        DatabaseMng.shared.changeSaveInAlbum(userID: currentUserID, friendID: friend.idFriend,  changeSetting: false) { isSaveInAlbum in
    ...
    @IBAction func saveInAlbumChanged(_ sender: UISwitch) {
        ...
        DatabaseMng.shared.changeSaveInAlbum(userID: currentUserID, friendID: friend.idFriend, changeSetting: true) { isSaveInAlbum in
    ...
    private func loadSharedMedia(){
        ...
        StorageMng.shared.downloadSharedMediaURLs(path: path) { result in
            switch result {
    ...
    private func getInitialMessage() {
        DatabaseMng.shared.getInitialMessage(with: friend.chatID) { message, passwordEncrypt in
...
//                    self!.friendProfileImageView.sd_setImage(with: url, completed: nil)
    ...
    private func createChat() {
        ...
        let encryptText = Encryption.shared.encryptDecrypt(oldMessage: "Hello back", encryptedPassword: passwordEncryption, messageID: dateString, encrypt: true)
        ...
        DatabaseMng.shared.sendChat(to: friend.chatID, usernameFriend: friend.usernameFriend, friendID: friend.idFriend, firstMessage: message, friendPhotoURL: friend.photoURLFriend, passwordEncrypt: passwordEncryption, isFriend: true, receiverAccepted: true) { [self] success in
            if success {
                
                DatabaseMng.shared.encryptInitialMessage(chatID: friend.chatID, passwordEncrypt: passwordEncryption, friendUsername: friend.usernameFriend , currentUsername: selfSender.displayName) { success in
                    print ("deleted the first 2 msgs")
                    if success {
                        self.statusUserFriend = .isFriend
                        
                        self.isFriend()
                        
                        DatabaseMng.shared.obtainToken(friendID: self.friend.idFriend) { token in
                            if !token.isEmpty {
                                self.sender.sendPushNotification(to: token, title: currentUsername, body: "Your contact accepted the request".localized(), typeNotification: "acceptNotification", chatFriend: nil)
                            }
                        }
                        self.navigationController!.popToRootViewController(animated: true)
                    }
    ...
    @IBAction func clearChatTapped(_ sender: Any) {
        ...
        let alert = UIAlertController(title: "Clear Chat".localized(), message: "Are you sure you want to erase chat".localized(), preferredStyle: UIAlertController.Style.alert)

        alert.addAction(UIAlertAction(title: "Clear".localized(), style: .default, handler: { (action: UIAlertAction!) in
            DatabaseMng.shared.clearChat(chatID: self.friend.chatID) { success in
                if success {
                    print ("Erased")
                    self.navigationController!.popToRootViewController(animated: true)
                }
...
//        }
    ...
    @IBAction func blockFriendTapped(_ sender: Any) {
        let alert = UIAlertController(title: "Block Contact".localized(), message: "Are you sure you want to block contact".localized(), preferredStyle: UIAlertController.Style.alert)

        alert.addAction(UIAlertAction(title: "Block".localized(), style: .default, handler: { (action: UIAlertAction!) in
            DatabaseMng.shared.blockFriend(chatID: self.friend.chatID , currentID: self.currentUserID, friendID: self.friend.idFriend, friendUsername: self.friend.usernameFriend, completion: { success in
                if success {
                    print ("Blocked")
                    self.navigationController!.popToRootViewController(animated: true)
                }
...
//        })
    ...
    @IBAction func cancelRequestTapped(_ sender: Any) {
        DatabaseMng.shared.deleteFriend(chatID: friend.chatID) { success in
            print (success)
        }
